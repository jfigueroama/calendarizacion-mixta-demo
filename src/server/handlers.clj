...
(defn trihorario-index
  ...
  (let [sname (case (:mode conf)
    ...
    (html5 [:head
            ...
            [:meta {:charset="UTF-8"}]
            (include-css "/public/css/horarios.css")
           ...
           [:body
            [:div#app {:data-entidad entidad
                       ...
                       :data-mode (:mode conf)
                       :data-ekey ekey
                       :data-data-url (str "/public/transit/" (:id horario))
                       :data-php-base-url (:admin-site-url conf)}
             
             [:i "Loading ..."]
             ]  ; App
            [:script {:src (str "/public/js/trihorario/edicion/" sname)} ]
  ...
  (let [sname (case (:mode conf)
    ...
    (html5 [:head
           ...
           [:body
            ...
            [:script {:src (str "/public/js/horario/edicion/" sname)} ]
    ...
    (if-let [h ($ dbc :horario id)]
      (html5 [:head
             ...
             [:body
              ...
              [:script {:src (str "/public/js/horario/edicion/app-devcards.js")} ]] ))))
